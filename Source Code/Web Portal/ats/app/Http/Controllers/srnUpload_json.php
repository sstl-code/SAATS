...
class srnUpload_json extends Controller
{
    public function getSrNData(Request $request)
    {
     ...
     //die("mmsssssm");
       // $result = DB::select(DB::raw('SELECT ats.insert_t_stn() as result'));
        ...
        $allUsersCount=DB::select('SELECT ats."insert_t_srn"()');
       // $result = DB::select($query->toString());
        //print_r($allUsersCount);die();
        // Return the data as a JSON response
        session()->flash('status', 'SRN Created Successfully .');
