...
class AuthenticationController extends Controller
{
  ...
  public function login(Request $request){
    ...
    $userDetials = DB::table('usr.v_user_login')->where(['tu_user_email'=>$email,'up_password'=>$password])->first();
    //dd(Auth::user());
    

    if(!empty($userDetials))
    {
      // $user = User::where('tu_user_email', $email)->first();

      // $accessToken = $user->createToken('authToken')->plainTextToken;
      // dd($accessToken);
      $insert = DB::table('usr.t_user_login_history')
                ->insert(
                    [
                        'ulh_user_login_time' => now(),
                        'ulh_latitude' =>$lat,
                        'ulh_longitude' => $lng,
                        'ulh_user_id' => $userDetials->tu_user_id,
